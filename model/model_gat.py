...
class ViGBlock(nn.Module):
    def __init__(self, in_features, hidden_features=None, drop=0.1, heads=4):
        super().__init__()
        hidden = hidden_features or in_features

        self.gat = GATConv(in_features, hidden, heads=heads, concat=True, dropout=drop, add_self_loops=False, negative_slope=0.1)
        self.fc  = nn.Linear(hidden * heads, in_features)
...
class ViGNN(nn.Module):
    def __init__(self, patch_emb=256, num_ViGBlocks=3, hidden_features=512,
        ...
        self.blocks = nn.ModuleList([
            ViGBlock(in_features=patch_emb, hidden_features=hidden_features, drop=0.1, heads=gat_heads)
    ...
    def forward(self, x):
        x = self.patchifier(x)                # [B, C, H, W]  (H=W=8)
        B, C, H, W = x.shape
        N = H * W
        ...
        x_flat = x_flat + self.pos_embedding[:, :N, :]  # [B, N, C]
...
class ViG_Classifier(nn.Module):
    def __init__(self, patch_emb=256, num_blocks=3, hidden_features=512, n_classes=9, k=5, dilation=1, gat_heads=4, grid_size=8):
        super().__init__()
        self.backbone = ViGNN(patch_emb=patch_emb, num_ViGBlocks=num_blocks,
                              hidden_features=hidden_features, k=k, dilation=dilation, gat_heads=gat_heads, grid_size=grid_size)
